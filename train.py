...
from deepsleep.trainer import DeepFeatureNetTrainer, DeepSleepNetTrainer, Seq2SeqNetTrainer
...
def train(n_epochs):
    trainer = Seq2SeqNetTrainer(
        data_dir=FLAGS.data_dir,
        output_dir=FLAGS.output_dir,
        n_folds=FLAGS.n_folds,
        fold_idx=FLAGS.fold_idx,
        batch_size=80,
        input_dims=FLAGS.n_features,
        n_classes=NUM_CLASSES,
        seq_length=5,
        n_rnn_layers=2,
        n_fc_layers=2,
...
def main(argv=None):
    ...
    if FLAGS.n_features:
        train(n_epochs=FLAGS.pretrain_epochs + FLAGS.finetune_epochs)
    else:
